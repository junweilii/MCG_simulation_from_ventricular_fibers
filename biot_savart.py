...
def mod_write_target_volume(input_coil: np.ndarray, input_current: np.ndarray, output_filename: str,
                        box_size: tuple, start_point: tuple,
                        coil_resolution: float = 1,
                        volume_resolution: float = 1) -> "tuple[np.ndarray]":

    coil, current = slice_coil(input_coil, input_current, coil_resolution)
    fields, positions  = produce_target_volume(
        coil, current, box_size, start_point, volume_resolution)
