...
#define _ion_cai	*(_ppvar[1].get<double*>())
#define _style_ca	*_ppvar[2].get<int*>()
...
static void nrn_alloc(Prop* _prop) {
 ...
 nrn_promote(prop_ion, 3, 0);
 	...
 	_ppvar[1] = &prop_ion->param[1]; /* cai */
 	_ppvar[2] = &(prop_ion->dparam[0].literal_value<int>()); /* iontype for ca */
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 Cadynam cadynam.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static const char *modelname = "intracellular Calcium concentration dynamic";
...
/*CVODE*/
 ...
 {
   Dcai = - 1e-7 * ics + 0.07 * ( 1e-7 - cai ) ;
   }
 return _reset;
}
 static int _ode_matsol1 () {
 Dcai = Dcai  / (1. - dt*( ( 0.07 )*( ( ( - 1.0 ) ) ) )) ;
  return 0;
}
 /*END CVODE*/
...
_deriv1_advance = 0;
 if(error){fprintf(stderr,"at line 38 in file cadynam.mod:\nBREAKPOINT {\n"); nrn_complain(_p); abort_run(error);}
    if (secondorder) {
    int _i;
    for (_i = 0; _i < 1; ++_i) {
...
static void register_nmodl_text_and_filename(int mech_type) {
    const char* nmodl_filename = "cadynam.mod";
