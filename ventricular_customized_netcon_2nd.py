...
# import geometric data of ventricle
data = sio.loadmat(current_dir + "\\data\\customized_left_ventricular_fiber_20latitude.mat")
...
x_spiral_key = 1000 * data['x_spiral_key']
y_spiral_key = 1000 * data['y_spiral_key']
z_spiral_key = 1000 * data['z_spiral_key']
...
for layer_ind in range(layer_num):
    ...
    for fiber_i in range(fiber_num):
        ...
        cell_num = np.size(x_spiral_interp[layer_ind][0][fiber_i, :], 0) - 1
        node[layer_ind][fiber_i] = [None] * cell_num

        for cell_ind in range(cell_num):
            # define the electrophysiology parameters of every myocyte
            ...
            # adjust the potassium conductance to make sure the action potential width of inner/outer ventricular layer are different
            if layer_ind == 0:
            ...
            node[layer_ind][fiber_i][cell_ind].pt3dadd(x_spiral_interp[layer_ind][0][fiber_i, cell_ind],
                                                       y_spiral_interp[layer_ind][0][fiber_i, cell_ind],
                                                       z_spiral_interp[layer_ind][0][fiber_i, cell_ind], cell_diameter)
            node[layer_ind][fiber_i][cell_ind].pt3dadd(x_spiral_interp[layer_ind][0][fiber_i, cell_ind + 1],
                                                       y_spiral_interp[layer_ind][0][fiber_i, cell_ind + 1],
                                                       z_spiral_interp[layer_ind][0][fiber_i, cell_ind + 1],
                                                       cell_diameter)
        ...
        # connect neighboring myocytes on a fiber with synapse
...
# this stimulus only apply to inner layer
ns = h.NetStim()
ns.interval = 1000
ns.number = 1
nc = [None] * fiber_num
...
# connect inner and outer layer myocytes with synapse
for fiber_i in range(fiber_num):
    ...
    nc[fiber_i] = [None] * (latitude_num - 1)
    syn[fiber_i] = [None] * (latitude_num - 1)
    ...
    for latitude_ind in range(latitude_num - 1):
        ...
        nc[fiber_i][latitude_ind] = [None] * layer_num
        syn[fiber_i][latitude_ind] = [None] * layer_num
        ...
        for layer_ind in range(layer_num):
            ...
            temp = abs(z_spiral_interp[layer_ind][0][gap_fiber_ind[fiber_i][latitude_ind][layer_ind], :] - gap_point_z)
            gap_cell_ind[fiber_i][latitude_ind][layer_ind] = np.where(temp == np.min(temp))
            gap_cell_ind[fiber_i][latitude_ind][layer_ind] = gap_cell_ind[fiber_i][latitude_ind][layer_ind][0][0]
            # because the number of interp point is one greater than cell number, the cell index should minus one
        ...
        for layer_ind in range(layer_num - 1):
            # innermost layer stimulus
            if layer_ind == 0:  # only add stimulus on inner layer
                syn[fiber_i][latitude_ind] = h.ExpSyn(node[layer_ind]
                                                      [gap_fiber_ind[fiber_i][latitude_ind][layer_ind]]
                                                      [gap_cell_ind[fiber_i][latitude_ind][layer_ind]](0.5))
                syn[fiber_i][latitude_ind].tau = 3  # ms, decay time constant
                syn[fiber_i][latitude_ind].e = 0  # mV, reversal potential
                nc[fiber_i][latitude_ind] = h.NetCon(ns, syn[fiber_i][latitude_ind])
                nc[fiber_i][latitude_ind].weight[0] = 1
                nc[fiber_i][latitude_ind].delay = 5

            # gap junction between neighboring layers
...
# calculate current from inner to outer
    ...
    for latitude_ind in range(latitude_num - 1):
        ...
        for layer_ind in range(layer_num-1):
            radial_current[fiber_i][latitude_ind][layer_ind] = (
                ...
                [gap_fiber_ind[fiber_i][latitude_ind][layer_ind + 1]]
                [gap_cell_ind[fiber_i][latitude_ind][layer_ind + 1]]))
            # gap junction conductance is 50nS, then we get current in nA
            radial_current[fiber_i][latitude_ind][layer_ind] = 50 * 1e-3 * gaussian_filter1d(radial_current[fiber_i][latitude_ind][layer_ind], sigma)

# save current from inner to outer
with open('radial_current_left_ventricle.pickle', 'wb') as handle:
    pickle.dump(radial_current, handle, protocol=pickle.HIGHEST_PROTOCOL)

# calculate current along every fiber
...
for layer_ind in range(layer_num):
    ...
    for fiber_i in range(fiber_num):
        ...
        for cell_ind in range(cell_num-1):
            ...
            # gap junction conductance is 50nS, then we get current in nA
            fiber_current[layer_ind][fiber_i][cell_ind] = 50 * 1e-3 * gaussian_filter1d(fiber_current[layer_ind][fiber_i][cell_ind], sigma)

        # save current along fiber
with open('fiber_current_left_ventricle.pickle', 'wb') as handle:
...
for fiber_i in range(fiber_num):
    for latitude_ind in range(latitude_num - 1):
        ...
        for layer_ind in range(layer_num-1):
            ...
            heart_point[layer_ind, :] = [x_gapJ/10000, y_gapJ/10000, z_gapJ/10000]  # um to cm
